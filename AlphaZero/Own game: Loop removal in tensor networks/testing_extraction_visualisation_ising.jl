...
include("game_v9_10x10_ising_test.jl")
include("params1.jl")


# Then, pass it to the experiment initialization:
experiment = AlphaZero.Experiment("ising_loops", GameSpec(), params, Network, netparams, benchmark)
...
function visualise_gameplay(state_traces)
    ...
    for (j, state) in enumerate(state_traces[2:end])
        ...
        display_selected_action_ising(old_graph, choosen_cycle, choosen_edge)
