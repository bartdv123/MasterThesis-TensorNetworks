...
GI.actions(::GameSpec) = [collect(1:7*18)]                                      # 7 loops in a frucht graph, 18 edges in a frucht graph
...
function update_action_mask!(env::GameEnv, action)                              # Mask for action which are not possible anymore after performing an action
    ...
    env.boolean_action = zero_padding_to_size(env.boolean_action, 7*18)
