...
p1 = AlphaZeroPlayer(Ses.env)
p2 = AlphaZero.RandomPlayer()

Ses = Session(experiment)
gspec = Ses.env.gspec
Trained_rewards = []
Trained_average_length = []
Random_rewards = []
...
for game in 1:num_games
    ...
    println("-------------------  TRAINED PLAYER $game  --------------------")

    trace = play_game(gspec, p1; flip_probability=0.)

    taken_path = last(trace.states).history
    loop_size1 = [length(taken[1]) for taken in taken_path]

    rewards_list1 = last(trace.states).reward_list
    reward1 =  sum(rewards_list1)

    push!(Trained_rewards, reward1)
    #11 loops
    push!(Trained_average_length, sum(loop_size1)/11)



    RandomPlayer
    println("-----------------------------   RANDOM PLAYER $game -----------------------")
...
println("trained rewards = ", Trained_rewards)
println("trained average length = ", Trained_average_length)
println("random rewards = ", Random_rewards)
println("random average length = ", Random_average_lengths)
