...
function GI.init(::GameSpec)
    ...
    history = Int[]
    return GameEnv(graph, fully_weighted_edge_list, sized_connections, Int64[], trues(length(fully_weighted_edge_list[1:10])), false, history)
end
...
function GI.clone(env::GameEnv)
    ...
    return GameEnv(deepcopy(env.graph), deepcopy(env.weighted_edge_list), 
...
GI.current_state(env::GameEnv) = (graph = env.graph, weighted_edge_list = (env.weighted_edge_list), 
